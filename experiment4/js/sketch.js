...
function setup() {
  ...
  let input = createInput("InfiniteQ*bert");
  input.parent(label);
  input.input(() => {
    rebuildWorld(input.value());
  });

  createP("WASD to scroll. Clicking changes tiles.").parent("canvas-container");
...
function resizeScreen() {
  ...
  resizeCanvas(canvasContainer.width()*(3/4), canvasContainer.height()*(3/4));
...
function draw() {
  ...
  for (let y = y0; y < y1; y++) {
    ...
    for (let x = x0; x < x1; x++) {
      drawTileGround(tileRenderingOrder([x + world_offset.x, y - world_offset.y]), [
        camera_offset.x,
        camera_offset.y
      ]);// odd row  
    }
...
function drawTileGround([world_x, world_y], [camera_x, camera_y]) {
  let [screen_x, screen_y] = worldToScreen(
    [world_x, world_y],
    [camera_x, camera_y]
  );
  push();
  translate(0 - screen_x, screen_y);
  if (window.p3_drawTileGround) {
    window.p3_drawTileGround(world_x, world_y, -screen_x, screen_y);
  }
  pop();
}
